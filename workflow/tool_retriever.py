...
class ToolRetriever:
    def __init__(
            ...
            llm_model: str = science_config.DeepSeekV3_2.model,
            base_url: str = science_config.DeepSeekV3_2.base_url,
            api_key: str = science_config.DeepSeekV3_2.api_key,
            max_per_category: int = 10,  # 减少数量，提高专注度
    ...
    def _build_domain_map_from_subset(self, subset_tools: List) -> Dict[str, List]:
        ...
        domain_map = {
            "biology": [t for t in biology_domain if t.name in subset_tool_names],
            "chemistry": [t for t in chemistry_domain if t.name in subset_tool_names],
            "materials": [t for t in material_domain if t.name in subset_tool_names],
            "general": subset_tools,
            "physics": subset_tools,
        }
