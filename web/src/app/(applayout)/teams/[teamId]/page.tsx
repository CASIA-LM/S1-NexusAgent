...
import { TeamsService, MembersService } from "@/client";
...
import MultiAgentSettings from "@/components/Teams/MultiAgentSettings";
    ...
    team && (
      ...
      >
        ...
        >
          ...
          team.workflow === "workflow_agent" ||
          team.workflow === "multi_agent" 
          ? (
            <Flex h="full" gap={4}>
              <Box
                flex={3}
        ...
        {team.workflow === "multi_agent" && (
          <Box mt={4}>
            {/* 调用 /flock/api/v1/teams/{teamId}/members/detail 接口获取 rootmember 详情信息 */}
            {/* 由于API可能需要通过自定义方式访问，这里使用MembersService获取成员列表并查找root成员 */}
            <MultiAgentSettings teamId={Number.parseInt(teamId)} />
          </Box>
        )}
      </Box>
    )
