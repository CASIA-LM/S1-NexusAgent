...
const ChatMain = ({ isPlayground }: { isPlayground?: boolean }) => {
  ...
  const stream = async (id: number, threadId: string, data: TeamChat) => {
    ...
    const streamPromise = fetchEventSource(url, {
      ...
      onclose() {
        console.log("SSE 连接关闭，不自动重试");
        return false;
      },
      onerror(error) {
        const errorMsg = error.message || "流式请求失败，请稍后重试";
        const errorStatus = error.status ? `（状态码：${error.status}）` : "";
        const fullErrorMsg = `${errorMsg}${errorStatus}`;
        showToast("请求错误", fullErrorMsg, "error");
        console.error("fetchEventSource 错误详情：", error);
        controller.abort();
